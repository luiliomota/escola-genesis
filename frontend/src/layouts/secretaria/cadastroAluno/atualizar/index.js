...
function Tables() {
    ...
    const [listaNacionalidade, setListaNacionalidade] = useState([{id:0, nome: ''}]);
    ...
    useEffect(() => {
        api.get(`/api/aluno/${idAluno}`)
          .then((response) => {
              ...
              const _aluno = response.data;
              _aluno.dataNascimento = moment(_aluno.dataNascimento, "DD/MM/YYYY");
              _aluno.dataMatricula = moment(_aluno.dataMatricula, "DD/MM/YYYY");
              _aluno.dataContrato = moment(_aluno.dataContrato, "DD/MM/YYYY");
              setAluno(_aluno);
          })
          .catch((error) => console.error(error))
    }, []);
    ...
    function handleSubmit() {
        api.put(`/api/aluno/${idAluno}`, aluno)
            .then((res) => {
                console.table(res);
                if (res.status === 201) {
                    openSuccessSB();
                            ...
                            <MDBox p={3} pb={3}>
                                <Grid container justifyContent='inherit' spacing={1}>
                                    ...
                                    <Grid item xs={12} md={3}>
                                        <MDBox mb={1}>
                                          <LocalizationProvider adapterLocale={moment.locale('pt-br')} dateAdapter={AdapterMoment}>
                                            <DesktopDatePicker
                                                fullWidth
