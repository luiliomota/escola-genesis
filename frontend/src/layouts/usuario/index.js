...
function Tables() {
  const { columns, rows, filtro, idUsuario, openDialog, setOpenDialog, setLista, setListaFiltro } = authorsTableData();
  const [searchText, setSearchText] = useState("");
    ...
    function handleDeleteSubmit() {
        api.delete((`/api/usuario/${idUsuario}`))
            .then((res) => {
                if(res.status === 200){
                    openSuccessSB("Usuario excluido com sucesso");
                    setOpenDialog(false);
                    atualizarLista();
                }
            }).catch((error) => {
