...
// Data
import authorsTableData from "pages/secretaria/aluno/data/authorsTableData";
...
function Tables() {
  ...
  const { columns, rows, filtro, idAluno, openDialog, setOpenDialog, setLista, setListaFiltro } = authorsTableData();
  const [searchText, setSearchText] = useState("");
    ...
    function handleDeleteSubmit() {
        api.delete((`/api/aluno/${idAluno}`))
            .then((res) => {
                if(res.status === 200){
                    openSuccessSB("Aluno excluido com sucesso");
                    setOpenDialog(false);
                    atualizarLista();
                }
            }).catch((error) => {
