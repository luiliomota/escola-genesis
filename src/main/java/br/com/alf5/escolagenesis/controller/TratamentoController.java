...
import br.com.alf5.escolagenesis.controller.dto.DetalhesTratamentoDto;
import br.com.alf5.escolagenesis.controller.dto.TratamentoDto;
import br.com.alf5.escolagenesis.controller.form.AtualizarTratamentoForm;
import br.com.alf5.escolagenesis.controller.form.TratamentoForm;
...
//    @Cacheable(value = "lista_tratamentos")
    public ResponseEntity<Page<TratamentoDto>> listaTratamentos(@RequestParam(required = false) String nomePaciente, Pageable paginacao,
        ...
        if (authentication.getAuthorities().contains(new Perfil(1L, "DIRECAO"))) {
            tratamentos = tratamentoRepository.findAll(paginacao);
        } else if (authentication.getAuthorities().contains(new Perfil(2L, "SECRETARIA")) ||
                    authentication.getAuthorities().contains(new Perfil(3L, "COORDENACAO")) ||
                    authentication.getAuthorities().contains(new Perfil(4L, "PROFESSOR"))) {
